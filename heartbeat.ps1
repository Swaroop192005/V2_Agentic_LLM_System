...
# independent of any terminal or Claude Code session. Restarts any of the core
# pipeline processes if it isn't running. This exists because the in-session
# bash watchdog (watchdog_v2.sh, launched via the Monitor tool) does NOT
# survive terminal closure, session teardown, or the machine sleeping - which
# caused three separate multi-hour/multi-day generation gaps during this
# project (see RESEARCH_LOG.md). Task Scheduler survives all of that as long
# as the user is logged in.
#
# generate_training_data_v2.py deliberately dropped from this list once the
# 5000-question dataset completed (RESEARCH_LOG Section 33/35) - restarting
# a finished generator every cycle just re-scans 5000 already-done questions
# and exits, wasted GPU/CPU churn forever. Re-add it here if generation is
# ever extended or restarted.
...
if ($restarted.Count -gt 0) {
    ...
    Add-Content $log "$(Get-Date -Format o)  ok (all 3 processes running)"
}
